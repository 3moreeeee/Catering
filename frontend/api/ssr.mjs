 ...
 * declared in src/server.ts — is routed here (see vercel.json) and handled
 * by the same Express app that `npm run serve:ssr:fk-catering` runs locally.
 * The bare `/` is routed here before the static files, because Vercel would
 * otherwise serve index.csr.html as the directory index and skip the redirect.
 * The original request URL is preserved through the route.
 */
